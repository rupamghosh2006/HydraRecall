...
export function normalizeBeamRecord(record) {
  if (!record || typeof record !== "object") throw new Error("BEAM record must be an object.");
  if (!record.question_id || !record.question || !Array.isArray(record.sessions)) throw new Error("BEAM records need question_id, question, and sessions array.");
  
  const mappedSessions = [];
  for (let i = 0; i < record.sessions.length; i++) {
    const rawSession = record.sessions[i];
    const turns = [];
    for (let j = 0; j < rawSession.length; j++) {
      const msg = rawSession[j];
      const timeAnchor = msg.time_anchor || `Session ${i + 1}`;
      turns.push({
        id: `t-${i}-${j}`,
        content: `[${timeAnchor}] ${msg.role.toUpperCase()}: ${msg.content}`,
        createdAt: new Date().toISOString()
      });
    }
    mappedSessions.push({
      id: `s-${i}`,
      title: `BEAM Session ${i + 1}`,
      createdAt: new Date().toISOString(),
      turns
    });
  }
...
export function buildBeamReaderPrompt(record, evidence) {
  const base = `You are evaluating an agent's memory capability on the BEAM benchmark.
Your task is to answer the following question strictly based on the provided conversation excerpts.
The question tests complex memory skills like multi-hop reasoning, contradiction resolution, or temporal tracking.
...
${evidence.map((e, idx) => `--- Excerpt ${idx + 1} from Session ${e.sessionId} ---\n${e.content}`).join("\n\n")}
[END EXCERPTS]`;

  return `${base}\n\nStrict Guidelines:\n- Answer the question solely using the context provided.\n- If the context lacks sufficient information, you MUST set "abstained": true and explain why in "reasoning".\n- Output valid JSON with keys: "abstained" (boolean), "answer" (string), and "reasoning" (string).\n`;
}
